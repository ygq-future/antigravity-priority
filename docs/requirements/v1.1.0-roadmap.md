...
**版本目标**：v1.1.0  
**状态**：本期需求已全部完成 / 待发布 (All Requirements Completed / Ready for Release)  
**创建日期**：2026-08-18  
**前置版本**：v1.0.1 (已发布)
...
2. [确立需求详细设计 (Detailed Specifications)](#2-确立需求详细设计-detailed-specifications)
   ...
   - [2.12 REQ-12：滑动窗口多样本自适应燃尽学习与样本容量动态配置](#212-req-12滑动窗口多样本自适应燃尽学习与样本容量动态配置)
...
7. [关键缺陷修复 (Bug Fixes)](#7-关键缺陷修复-bug-fixes)

---

## 1. 需求总览 (Requirements Summary)

> **注**：截止当前版本，v1.1.0 规划的全部 12 项核心需求及 3 项关键缺陷修复已 100% 开发完毕并通过全量测试验收。

| 编号 | 需求项 | 模块 | 优先级 | 研发状态 | 核心价值 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **REQ-01** | 下次自动调度倒计时实时刷新与自适应时间格式 | 前端 / 后端 | P1 | ✅ 已完成 | 消除静态字符串显示，提供秒级自适应倒计时体验 |
| **REQ-02** | 插件配置参数大小写容错与全字段健壮兜底 | 配置解析 | P0 | ✅ 已完成 | 杜绝配置误填（如 `30M`）导致插件启动崩溃或停止服务 |
| **REQ-03** | 自定义模态 Confirm 对话框替换浏览器原生 Confirm | 前端 UI | P1 | ✅ 已完成 | 适配 CPA Host 双主题与设计系统，提升整体视觉质感 |
| **REQ-04** | 凭证列表常驻展示、配额缓存按需探测与两阶段 Dry-Run | 调度与交互 | P0 | ✅ 已完成 | 规避频繁探测 Google 配额接口导致的风控与 429 限制 |
| **REQ-05** | 单 HTTP 请求提取全组配额与跨模型组预测优先级即时切换 | 核心引擎 | P0 | ✅ 已完成 | 一次请求同时获取 Gemini 与 Claude/GPT 额度，支持即时切换预测视图 |
| **REQ-06** | UI 布局优化、独立滚动容器、紧凑双列卡片与平滑动效 | 前端 UI/动效 | P1 | ✅ 已完成 | 消除页面整体滚动，支持单行/双列视图切换，提供复制与细腻动效 |
| **REQ-07** | 执行历史变更明细对比弹窗 (Apply 变更 & Dry-Run 预测对比) | 诊断与历史 | P1 | ✅ 已完成 | 支持回溯历史调度详情，清晰对比 Apply 实际写回与 Dry-Run 前后配额差异 |
| **REQ-08** | 自动调度时间区间限制、一键手动暂停/恢复与面板级持久化控制 | 调度核心 / UI | P0 | ✅ 已完成 | 支持在 UI 直接一键启停调度与配置生效时段（如夜间/工作时间），免改 YAML 重启 |
| **REQ-09** | UI 动态配置中心与运行时免重启热生效 | 配置 / UI / 核心 | P0 | ✅ 已完成 | 将所有非必须重启的业务调度配置迁移至独立 UI 配置中心，极简 CPA YAML 并实现秒级热生效 |
| **REQ-10** | 相近分值平级分档与原生负载均衡 | 优先级规划 / 核心 | P0 | ✅ 已完成 | 基于紧迫度容差将相近账号聚类分配相同优先级，由 CPA 原生轮询分流，消除单点打穿 429 |
| **REQ-11** | 429 速率限制实时拦截与熔断冷却机制 | 核心 / 拦截器 / 状态 | P0 | ✅ 已完成 | 实时捕获业务调用 429 错误，立即将账号降级至 `-1` 兜底队列，到期自动自愈恢复 |
| **REQ-12** | 滑动窗口多样本自适应燃尽学习与样本容量动态配置 | 状态估计 / 配置 / UI | P0 | ✅ 已完成 | 采用多样本时序滑动队列累积增量，彻底消除小步慢跑被吞问题，并支持 UI 动态配置容量 (2-30) |
...
#### 管理 API 接口扩展
- `GET /v0/management/plugins/antigravity-priority/runtime-config`：获取当前完整运行时配置。
- `POST /v0/management/plugins/antigravity-priority/runtime-config`：提交并热应用新配置。
...
### 2.12 REQ-12：滑动窗口多样本自适应燃尽学习与样本容量动态配置

#### 现状与痛点
原本自适应学习率采用两两探测相邻快照对比机制（即只保留最新 1 条记录，两两相减）。当用户调用频次较低、单次探测间隔（如 15 分钟）内的 5h 短窗消耗量小于 $5\%$（$\Delta R_{\text{5h}} < 0.05$）时，相邻两点无法触发自适应学习阈值，而每次探测又会直接覆盖上一条记录，导致小步慢跑消耗被吞、学习率长期停留在默认值 `0.15`。

#### 技术设计
1. **多样本时序滑动窗口队列 (Sliding Window FIFO Queue)**：
   - 在持久化 `Entry` 中扩充 `Samples []QuotaSample`（记录 `ObservedAt`、`ShortWindowResetAt`、`ShortWindowRem`、`LongWindowRem`）；
   - 容量由动态配置 `quota_sample_capacity` 控制（默认 `6` 条，支持 `2 ~ 30` 范围可配）。
2. **三重防御性生命周期维护**：
   - **窗口重置清空 (Reset Eviction)**：当检测到 5h 短窗重置时间变更或额度增加时，清空 `Samples` 队列，并将当前点设为新周期的起始基准点；
   - **零消耗去重 (Zero Consumption Deduplication)**：当配额无任何变化时，仅刷新最新样本的时间戳，不向队列追加重复点，防止有效样本被无用数据挤出；
   - **容量截断 (FIFO Sliding)**：超出配置容量时按 FIFO 弹出最老样本，保持队列长度稳定。
3. **跨周期跨度累积学习**：
   - 以队列首个样本 $S_0$ 与当前样本 $S_{\text{curr}}$ 计算跨度消耗 $\Delta R_{\text{5h}} = (S_0.R_{\text{5h}} - S_{\text{curr}}.R_{\text{5h}})/100$；
   - 当累积消耗 $\Delta R_{\text{5h}} \ge 0.05$ 且 $\Delta R_{\text{7d}} > 0$ 时，计算瞬时比率 $obs = \Delta R_{\text{7d}} / \Delta R_{\text{5h}}$，经过 $[0.08, 0.30]$ 截断并按 $\alpha=0.3$ EMA 更新 $C_{\text{cycle}}$，并将当前点设为新基准，保证消耗量被精准学习且不重复计算。
4. **多模型组完全独立隔离**：
   - Gemini 组与 Claude/GPT 组在持久化 Store 中各维护独立的 `Entry`、独立的 `Samples` 时序切片与独立的自适应燃烧率。
5. **UI 配置中心可视化调节**：
   - 在 Web UI 配置中心的“探测与写回性能”卡片中提供**“自适应时序样本容量”**输入框（2-30，默认 6），支持免重启热生效。
...
### 5.1 完整运行时配置接口 (`GET/POST /v0/management/plugins/antigravity-priority/runtime-config`)
```json
{
  "auto_apply": true,
  "interval": "15m",
...
| **Multi-Sample Estimator** | 算法/状态测试 | 验证小步慢跑多周期累积触发、零消耗去重、5h 重置清空基准与 FIFO 容量截断；验证动态配置 2..30 热更新 |
