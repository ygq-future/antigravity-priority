...
| 配置项 | 默认值 | 范围/选项 | 说明 |
| :--- | :--- | :--- | :--- |
| **自动定时调度 (`auto_apply`)** | `true` | 开 / 关 | 是否由后台定时器周期性自动执行探测、规划并写回宿主凭证优先级。 |
...
| **429 熔断冷却时长 (`rate_limit_cooldown_minutes`)** | `5` | `1 ~ 60` (分钟) | 遭遇 429 限流时临时降级至 `-1` 兜底队列的冷却期，到期自动自愈。 |
| **启用双窗口优先级规则 (`priority_rules.enabled`)** | `true` | 开 / 关 | 是否启用基于 5h 短窗与 7d 长窗的双窗口智能优先级决策。 |
| **动态提权起始优先级 (`boost_start_priority`)** | `999` | `1 ~ 999` | 触发提权状态的第一梯队基准起始优先级。 |
| **常规健康起始优先级 (`normal_start_priority`)** | `100` | `1 ~ 999` | 常规可用健康梯队的基准起始优先级。 |

> **持久化保障**：所有通过 UI 配置中心修改的选项会自动原子保存至 `data/antigravity-priority-cache.json`，重启 CPA 容器数据不丢失，且优先级高于 YAML 初始值。
